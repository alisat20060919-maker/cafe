(() => {
  function ensurePages(){
    ...
    style.textContent = `
      .game-window.page-mode .location-tabs,.game-window.page-mode .map-area,.game-window.page-mode .cafe-inside,.game-window.page-mode .dialogue-box,.game-window.page-mode .gacha-spa{display:none!important}
  ...
  function showHome(){
    ...
    win?.classList.remove('page-mode','gacha-mode','inside-mode');
    document.querySelectorAll('.bottom-page').forEach(p=>p.classList.remove('active'));
    document.querySelectorAll('.bottom-dock button').forEach(b=>b.classList.remove('on'));
  }
  function openPage(name, btn){
    ensurePages();
    const already = document.querySelector('.game-window')?.classList.contains('page-mode') && document.querySelector(`.bottom-page[data-page="${name}"]`)?.classList.contains('active');
    if (already) { showHome(); return; }
  ...
  function wire(){
    ...
    if (!dock || dock.dataset.pagesV2) return;
    dock.dataset.pagesV2 = '1';
    const old = [...dock.querySelectorAll('button')];
    old.forEach((btn,i)=>{ if(i!==1){ const n=btn.cloneNode(true); btn.replaceWith(n); }});
    const btns=[...dock.querySelectorAll('button')];
    const sign=btns[0], gacha=btns[1], inventory=btns[2], commission=btns[3], settings=btns[4];
    sign?.addEventListener('click',e=>{e.preventDefault();openPage('sign',sign);});
    inventory?.addEventListener('click',e=>{e.preventDefault();openPage('inventory',inventory);});
    commission?.addEventListener('click',e=>{e.preventDefault();openPage('commission',commission);});
    settings?.addEventListener('click',e=>{e.preventDefault();openPage('settings',settings);});
    gacha?.addEventListener('click',()=>{document.querySelector('.game-window')?.classList.remove('page-mode');document.querySelectorAll('.bottom-page').forEach(p=>p.classList.remove('active'));});
  }
  wire();
