...
import { initCafeRoomV2 } from './cafe-room-v2.js?v=room004';
import { initCafeRoomFollowFix } from './cafe-room-follow-fix.js?v=room005';
...
function renderDropTextSummary(locationId = '') {
  ...
  const eventText = eventCount > 0
    ...
    : '這個地點目前沒有特殊事件。';
  return `<div class="gather-rate-text-summary"><p><b>主要會獲得：</b>${escapeHtml(joinNames(groups.common || []))}。</p><p><b>也有機會獲得：</b>${escapeHtml(joinNames(groups.normal || []))}。</p><p><b>較少見素材：</b>${escapeHtml(rareNames)}。</p><p><b>特殊事件：</b>${escapeHtml(eventText)}</p><p class="core-muted">實際結果仍依採集表隨機抽取；這裡只用文字概述，不再顯示完整百分比清單。</p></div>`;
}

function showDropRateModal(locationId = '') {
  const title = getSceneTitle(locationId);
  showModal(`<div class="core-modal-card compact gather-rate-modal"><button type="button" class="core-modal-close" data-close-modal>×</button><span class="core-modal-kicker">GATHER HINT</span><h2>${title}採集提示</h2><p>素材數量變多後，這裡改成簡短文字說明，不再列出全部掉落率。</p>${renderDropTextSummary(locationId)}</div>`);
}
...
function boot() {
  ...
  if (!edgeReport.skipped) showModal(renderEdgeTestReport(edgeReport));
  else if (!smokeReport.skipped) showModal(renderSmokeTestReport(smokeReport));
  else showOpeningStoryIfNeeded();
}
