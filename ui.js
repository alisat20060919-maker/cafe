...
function handleModalClick(event) {
  if (event.target === modalHost || event.target.closest('[data-close-modal]')) {
    closeModal();
    return;
  }

  const openingButton = event.target.closest('[data-opening-story-next]');
  if (openingButton) {
    const next = openingButton.dataset.openingStoryNext;
    if (next === 'finish') {
  ...
  if (event.target.closest('[data-confirm-reset-save]')) {
    resetState();
    closeModal();
...
export function initUI() {
  ...
  if (modalHost.dataset.eventsBound !== 'true') {
    modalHost.dataset.eventsBound = 'true';
...
function openImportSaveModal() { showModal(`<div class="core-modal-card"><button type="button" class="core-modal-close" data-close-modal>×</button><span class="core-modal-kicker">SAVE IMPORT</span><h2>匯入存檔</h2><textarea id="saveImportText" placeholder="把匯出的存檔文字貼在這裡"></textarea><div class="core-setting-list"><button type="button" data-confirm-import-save>匯入存檔</button><button type="button" data-close-modal>取消</button></div><p class="core-modal-note">匯入後會覆蓋目前這台裝置的本機存檔。</p></div>`); }
function openResetSaveModal() { showModal(`<div class="core-modal-card compact"><button type="button" class="core-modal-close" data-close-modal>×</button><span class="core-modal-kicker">RESET SAVE</span><h2>清除存檔？</h2><p>這會重置精靈咖啡屋的本機存檔。清除後不能復原，建議先匯出備份。</p><div class="core-setting-list"><button type="button" data-confirm-reset-save>確定清除</button><button type="button" data-close-modal>取消</button></div></div>`); }
