...
function renderFilterTabs() {
  return GameDB.getInventoryCategories()
    .map((category) => `<button type="button" data-filter="${category.id}" class="${currentInventoryFilter === category.id ? 'active' : ''}"><span>${category.icon}</span>${category.label}</button>`)
    .join('');
}

function renderRarityTabs() {
  return GameDB.getInventoryRarities()
    .map((rarity) => `<button type="button" data-rarity-filter="${rarity.id}" class="${currentInventoryRarity === rarity.id ? 'active' : ''}"><span>${rarity.icon}</span>${rarity.label}</button>`)
    .join('');
}
...
export function renderInventory() {
  ...
  const itemCards = Object.values(GameDB.items)
    ...
    .map(({ item, count, index }) => `
      <article class="core-item-card rarity-${item.rarity.toLowerCase()}" data-category="${item.type}" data-rarity="${item.rarity}" data-search="${escapeAttr(GameDB.getItemSearchText(item))}" data-search-match="true" data-sort-default="${index}" data-sort-rarity="${GameDB.getRarityRank(item.rarity)}" data-sort-type="${GameDB.getItemTypeRank(item.type)}" data-sort-count="${count}" data-sort-name="${escapeAttr(item.name)}">
        ...
        <div><b>${item.name}</b><span>${GameDB.getRarityLabel(item.rarity)} / ${GameDB.getItemTypeLabel(item.type)} / ${'★'.repeat(item.stars)}</span><p>${item.description}</p><div class="core-item-meta"><small>來源：${GameDB.getItemSourceText(item)}</small><small>用途：${item.use}</small></div><button type="button" class="core-detail-button" data-detail-kind="item" data-detail-id="${item.id}">查看詳情</button></div>
        <strong>×${count}</strong>
      </article>`)
    .join('');
  ...
  const fairyCards = Object.entries(state.fairies)
    ...
    .map(([fairyId], index) => {
      const fairy = GameDB.fairies[fairyId];
      if (!fairy) return '';
      return `<article class="core-item-card ssr" data-category="fairy" data-rarity="${fairy.rarity}" data-search="${escapeAttr(GameDB.getFairySearchText(fairy))}" data-search-match="true" data-sort-default="${fairyDefaultOffset + index}" data-sort-rarity="${GameDB.getRarityRank(fairy.rarity)}" data-sort-type="${GameDB.getItemTypeRank('fairy')}" data-sort-count="1" data-sort-name="${escapeAttr(fairy.name)}"><div class="core-item-icon">${fairy.icon}</div><div><b>${fairy.name}</b><span>${GameDB.getRarityLabel(fairy.rarity)} / ${GameDB.getItemTypeLabel('fairy')}</span><p>「${fairy.quote}」</p><div class="core-item-meta"><small>來源：${GameDB.getFairySourceText(fairy)}</small><small>${fairy.description}</small></div><button type="button" class="core-detail-button" data-detail-kind="fairy" data-detail-id="${fairy.id}">查看詳情</button></div><strong>已契約</strong></article>`;
    })
    .join('');
  const hasInventoryContent = Boolean(itemCards || fairyCards);
  page.innerHTML = `
    ${pageHeader('BAG / RENDER FROM STATE', '背包', '這裡讀取 gameState.inventory 和 gameState.fairies 動態生成；篩選條件會保留，不會每次重繪就重置。')}
    <div class="core-actions-row collection-entry-actions"><button type="button" data-route="collection">打開圖鑑</button></div>
    ${hasInventoryContent ? `
      <div class="core-search-box"><label for="inventory-search">搜尋背包</label><input id="inventory-search" type="search" value="${escapeAttr(currentInventoryQuery)}" placeholder="輸入素材、甜點、稀有度、來源或用途" autocomplete="off" /></div>
      <div class="core-sort-box"><label for="inventory-sort">排序</label><select id="inventory-sort">${renderSortOptions()}</select></div>
      <div class="core-filter-group"><p>分類</p><div class="core-filter-tabs" aria-label="背包分類篩選">${renderFilterTabs()}</div></div>
      <div class="core-filter-group"><p>稀有度</p><div class="core-filter-tabs" aria-label="背包稀有度篩選">${renderRarityTabs()}</div></div>
      <div class="core-list" id="inventory-list" data-current-filter="${currentInventoryFilter}" data-current-rarity="${currentInventoryRarity}" data-search-mode="all">${itemCards}${fairyCards}<p class="core-empty core-search-empty" id="inventory-search-empty" hidden>沒有符合目前條件的物品。</p></div>
